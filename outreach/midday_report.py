...
def gather_midday_data() -> dict:
    ...
    projekt = c.get("projekt_sent", 0)
    total = partner + referral + bauherr + projekt
    limit = _total_limit()
    projekt_on = config.PROJEKT_ENABLED and config.PROJEKT_SEND_ENABLED
    projekt_rem = max(0, config.PROJEKT_DAILY_SEND_LIMIT - projekt) if projekt_on else 0
    ...
    return {
        ...
        "projekt_sent": projekt,
        "projekt_limit": config.PROJEKT_DAILY_SEND_LIMIT if projekt_on else 0,
        "projekt_remaining": projekt_rem,
        "projekt_queued": summary.get("projekt_queued", 0),
        "projekt_total": summary.get("projekt_sent_all_time", 0),
        "projekt_on": projekt_on,
        "projekt_ref": AKTUELL.referenz,
        "projekt_region": AKTUELL.region,
        "replies_today": len(storage.replies_on_day(today.isoformat())),
...
def build_midday_email(data: dict) -> tuple[str, str, str]:
    ...
    if data["projekt_on"]:
        subject = (
            f"📊 Mittags-Update · Duisburg {data['projekt_sent']} Betriebe angeschrieben "
            f"· gesamt {data['total_sent']}/{data['total_limit']}"
        )
    else:
        subject = (
    ...
    projekt_text = (
        f"""
BAUVORHABEN {data['projekt_region'].upper()} ({data['projekt_ref']})
• Heute angeschrieben:  {data['projekt_sent']} / {data['projekt_limit']}  (noch {data['projekt_remaining']})
• Insgesamt:            {data['projekt_total']} Betriebe
• Noch in der Liste:    {data['projekt_queued']}
• ANTWORTEN heute:      {data['replies_today']}  (gesamt {data['replies_total']})
"""
        if data["projekt_on"] else ""
    )
    projekt_html = (
        f"""<div style="background:#fff4e5;border-left:4px solid #d98324;border-radius:6px;padding:16px;margin:16px 0">
  <p style="margin:0 0 6px;font-weight:700;color:#8a4b00">Bauvorhaben {data['projekt_region']} · {data['projekt_ref']}</p>
  <p style="margin:0;font-size:28px;font-weight:700;color:#8a4b00">{data['projekt_sent']} Betriebe heute</p>
  <p style="margin:4px 0 0;color:#8a6a44">Insgesamt {data['projekt_total']} angeschrieben · noch {data['projekt_queued']} in der Liste</p>
  <p style="margin:10px 0 0;font-size:20px;font-weight:700;color:#0b3d2e">{data['replies_today']} Antworten heute · {data['replies_total']} gesamt</p>
</div>"""
        if data["projekt_on"] else ""
    )
...
<tr><td style="padding:8px;border-bottom:1px solid #eee">Bauvorhaben Duisburg</td><td style="padding:8px;text-align:right;border-bottom:1px solid #eee"><strong>{data['projekt_sent']}</strong> / {data['projekt_limit']}</td><td style="padding:8px;text-align:right;border-bottom:1px solid #eee">{data['projekt_remaining']}</td></tr>
